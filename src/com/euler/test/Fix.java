...
public class Fix extends F{
    ...
    public static String a(String str) {
        Log.d("qq", str);
        Log.i("euler", "fix success");
        ActivityManager am = (ActivityManager) App.getContext()
                .getSystemService("activity");
        List<RunningTaskInfo> list = am.getRunningTasks(1);
        Log.i("euler", "list.size="+list.size());
        if (null != list && 0 < list.size()) {
            RunningTaskInfo info = list.get(0);
            Log.d("euler", "pkg=" + info.topActivity.getPackageName());
            Log.d("euler", "class=" + info.topActivity.getClassName());
        }
        return "b";
    }
